...
public class TerminalPageUtils {
	...
	public static String getActualPixelOutput(Page page, String language) {
		...
		case "Pixel" -> {
			...
			yield page.locator(OUTPUT_XPATH.replace("{type}", "ace_string")).textContent().trim().replace("\"", "");
		}
		case "Python" -> {
			page.locator(OUTPUT_XPATH.replace("{type}", "ace_constant"))
					.waitFor(new Locator.WaitForOptions().setState(WaitForSelectorState.VISIBLE));
			yield page.locator(OUTPUT_XPATH.replace("{type}", "ace_constant")).textContent().trim();
		}
	...
	public static void changeToLanguage(Page page, String language) {
		...
		if (cookiePopupFlag) {
			...
			popupAccept.scrollIntoViewIfNeeded();
			AICorePageUtils.waitFor(popupAccept);
			popupAccept.click();
		}
		// Change the language by clicking on the language logo
		try {
			Locator languageLocator = page.getByTitle(language);
			AICorePageUtils.waitFor(languageLocator);
