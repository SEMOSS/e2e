...
public class ViewExistingFunctionsOnCatalogPageTests extends AbstractPlaywrightTestBase {
	...
	void setup(@PWPage Page page) {
		...
		for (String tag : List.of("embeddings", "Test1")) {
			EditMetadataPageUtils.enterTagName(page, tag);
		}
		for (String domainName : List.of("SAP", "AI")) {
			EditMetadataPageUtils.enterDomainName(page, domainName);
		}
		for (String option: List.of("IP", "PHI")) { // data classification options
			EditMetadataPageUtils.selectDataClassificationOption(page, option);
		}
		for (String option : List.of("IP ALLOWED", "PHI ALLOWED")) {
		...
		Assertions.assertEquals(actualToastMessage, "Successfully set the new metadata values for the engine");
	}
	...
	void testFilterFunctionalityDiscoverableFunctionsTab(String filterCategoryName, String filterValues,
		...
		FunctionAccessSettingsUtils.clickOnMakeDiscoverableButton(page,
				TestResourceTrackerHelper.CATALOG_TYPE_FUNCTION);
		logout(page);
		loginEditor(page);
		MainMenuUtils.openMainMenu(page);
		MainMenuUtils.clickOnOpenFunction(page);
	...
	private static Stream<Arguments> provideContentToFunctionCatalogCard() {
		return Stream.of(Arguments.of("lock, bookmark, view logs dashboard, delete"));
	}
